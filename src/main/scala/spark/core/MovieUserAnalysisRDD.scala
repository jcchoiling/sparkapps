...
//      .foreach(println)
    ...
    //(UserID,(UserID,MovieID,Rating))
    val genderFilteredRatings = ratings.map(x => (x._1,(x._1,x._2,x._3)))
    val usersGenderBasic = usersRDD.map(_.split("::")).map (user => (user(0),user(1)))
    val genderRatings = genderFilteredRatings.join(usersGenderBasic).cache() //(2828,((2828,3793,3),M))
